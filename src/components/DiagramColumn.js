...
class DiagramColumn extends Component {
    ...
    render() {
        ...
        var knownWords = known.map ( (word) => <div><p>{word.word}</p><div className="diagram-tooltip">{word.definition}</div></div> )
        var unknownWords = unknown.map ( (word) => <div><p>{word.word}</p><div className="diagram-tooltip">{word.definition}</div></div>  )
